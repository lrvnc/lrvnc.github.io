      ...
      >
        <motion.div variants={itemVariant} className="mb-6">
          <div className="w-40 h-40 sm:h-56 sm:w-56 md:h-64 md:w-64 lg:h-64 lg:w-64 xl:w-64 xl:h-64 rounded-full overflow-hidden border-4 border-gray-700 shadow-lg">
            <img src={personImg} alt={heroData.name} className="w-full h-full object-cover" />
          </div>
        </motion.div>

        <motion.h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-5xl font-bold px-4" variants={itemVariant}>{heroData.name}</motion.h1>
        <motion.p className="text-base sm:text-lg md:text-xl lg:text-xl mt-2 lg:mt-4 md:mt-4 sm:mt-4 text-gray-300 px-4" variants={itemVariant}>{heroData.role}</motion.p>
        ...
        <motion.div className="mt-4 flex flex-col sm:flex-row justify-center items-center gap-4" variants={itemVariant}>
          ...
          <Button size="lg" asChild className="w-full bg-gray-800/60 hover:bg-gray-800/90 border border-white/60 transition-transform duration-200 hover:scale-105">
            <a href={heroData.cvPdf} download="Leandro_RV.pdf">
        ...
        <motion.div className="mt-8 flex justify-center items-center gap-6" variants={itemVariant}>
          <a href={heroData.socials.linkedin} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors"><Linkedin size={24} /></a>
          <a href={heroData.socials.github} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors"><Github size={24} /></a>
          <a href={heroData.socials.orcid} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors" aria-label="ORCID">
            <PenSquare size={24} />
          </a>
          <a href={`mailto:${heroData.email}`} className="text-gray-400 hover:text-white transition-colors" aria-label="Email">
            <Mail size={24} />
          </a>
        </motion.div>
      </motion.div>
